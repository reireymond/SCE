...
#include "view/fornecedor/fornecedor_view.h" // NOVO: Include da view de fornecedor
#include "controller/transacao/transacao_controller.h"

// Funcao auxiliar: transforma data (DD/MM/AAAA) e hora (HH:MM) em um numerão (AAAAMMDDHHMM)
...
void adicionarEventoController(Sistema *sistema) {
    ...
    // --- Parte de adicionar fornecedores (parceiros) ---
    // NOVO: Adicionei essa parte aqui pro TP
    printf("\nQuer adicionar servicos de terceiros (Buffet, Seguranca...)? (1-Sim, 0-Nao): "); 
    scanf("%d", &opcao);
    limpar_buffer();
    ...
    while(opcao == 1) {
        ...
        listarFornecedoresView(sistema); // Mostra a lista pra escolher
        int codigo_fornecedor; 
        printf("Codigo do Fornecedor: "); 
        scanf("%d", &codigo_fornecedor);
        limpar_buffer();
        
        Fornecedor *fornecedor_encontrado = NULL;
        // Procura o fornecedor pelo ID
        for(int i=0; i < sistema->num_fornecedores; i++) {
            if(sistema->lista_fornecedores[i].codigo == codigo_fornecedor) {
                fornecedor_encontrado = &sistema->lista_fornecedores[i];
            }
        }

        if(fornecedor_encontrado != NULL) {
            float valor_cobrado;
            printf("Quanto esse fornecedor cobrou pelo servico? R$ ");
            ler_float_positivo(&valor_cobrado);

            // Aumenta a lista
            novo->lista_fornecedores_alocados = realloc(novo->lista_fornecedores_alocados, (novo->num_fornecedores_alocados + 1) * sizeof(ItemFornecedorEvento));
            
            ItemFornecedorEvento *item = &novo->lista_fornecedores_alocados[novo->num_fornecedores_alocados];
            item->codigo_fornecedor = fornecedor_encontrado->codigo;
            item->valor_cobrado = valor_cobrado;
            
            novo->num_fornecedores_alocados++;
            novo->custo_total_previsto += valor_cobrado; // Soma no total do orcamento
            printf("Fornecedor adicionado!\n");
        } else {
            printf("Fornecedor nao achado.\n");
        }
        printf("Adicionar mais fornecedores? (1-Sim, 0-Nao): "); 
        scanf("%d", &opcao);
        limpar_buffer();
    }
